...
object MatMultiply{
  def multiMat[T: ClassTag](
    ...
    //拆分矩阵中，每个小矩阵的维度设定为2000
    val blockSize = 2000
    //如果维度少于2000,就没有必要再做矩阵拆分计算了
  ...
  def main(args: Array[String]){
    if (args.length != 3){
      System.err.println("Usage: MatMultiply <InputFile1> <InputFile2>  <Output>")
      System.exit(1)
    }
    ...
    val mat1=text1.map{x => val line=x.split(",");(line(1),(line(0),line(2).toDouble))}
    val mat2=text2.map{x => val line=x.split(",");(line(0),(line(1),line(2).toDouble))}
    multiMat(sc, mat1, mat2).map(x => x._1._1+","+x._1._2+","+x._2.toString).saveAsTextFile(args(2))
  }
}