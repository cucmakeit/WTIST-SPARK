...
object Similarity{
  ...
  def Jaccard(array1:Array[String], array2:Array[String]):Double={
    ...
    var j = 0;
    while (i<len1 && j < len2){
      if(array1(i).compareTo(array2(j))< 0) i += 1
      else if(array1(i).compareTo(array2(j))> 0) j += 1
      else {
        i += 1
        j += 1
        intersection += 1
  ...
  def simEst(a:String, b:String, method:String):Double={
    ...
    val array2 = b.split(",")
    if (method == "ecludSim"){
      val vec1 = DenseVector(array1.map(_.toDouble))
      val vec2 = DenseVector(array2.map(_.toDouble))
      return 1.0/(1.0 + norm(vec1 - vec2))
    }
    else if(method == "pearsSim"){
      if(array1.length<3) return 1.0
      return 0.5+0.5*corrcoef(array1.map(_.toDouble).view, array2.map(_.toDouble).view)
    }
  ...
  def main(args: Array[String]){
    if (args.length != 4){
      System.err.println("Usage: Similarity <InputFile> <Separator> <EstMethod[ecludSim,pearsSim,cosSim,Jaccard]> <Output>")
      System.exit(1)
    }
    if (!Array("ecludSim","pearsSim","cosSim","Jaccard").exists(s => s == args(2))){
